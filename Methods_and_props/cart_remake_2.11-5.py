...
class Cart:
    ...
    def add(self, product, quantity=1):
        if isinstance(product, Product):
            if self.goods.get(product) is None:
                self.goods[product] = quantity
                self.__total += product.__dict__['price'] * quantity
            else:
                self.__total -= self.goods[product] * product.__dict__['price']
                self.goods[product] = quantity
                self.__total += product.__dict__['price'] * quantity
        else:
    ...
    def print_check(self):
        print(f'---Your check---')
        for key in sorted(self.goods, key=lambda x: x.name):
            print(f"{key.__dict__['name']} {key.__dict__['price']} {self.goods[key]} {key.__dict__['price'] * self.goods[key]}")
